...
If exists(Select 1 from sys.objects where name = 'getDiskSpaceInfo' and type = 'P')
DROP PROCEDURE [dbo].[getDiskSpaceInfo]
GO
CREATE PROCEDURE [dbo].[getDiskSpaceInfo]
AS
BEGIN
SELECT DISTINCT
...
CAST(CAST(available_bytes AS FLOAT)/ CAST(total_bytes AS FLOAT) AS DECIMAL(18,2)) * 100 AS [Available Space In %]
